function rand([min = 0, max = 1000]) { //criando uma funçao rand que vai retornar um valor entre 0 e 1000 sendo que este parâmetro esta em [] que é um operador destructuring.
    if (min > max) [min, max] = [max, min] //se o valor do min foi maior que max ele vai trocar min por max
    const valor = Math.random() * (max - min) + min // criando constante valor que recebe Math.random() que vai perar um valor aleatório, vezes o maximo - o minimo + minimo para criar um intervalo
    return Math.floor(valor) // floor arredonda para baixo
}

console.log(rand([50, 40])) //escreva a função rand usando arrey como parâmetro
console.log(rand([995])) //escreva  função rand usando o valor [min: 995] como parâmetro arrey. o max fica como 1000 pois foi declarado acima
console.log(rand([])) //escreva a função rand com o parâmetro []Arrey vazio, de 0 a 1000
//console.log(rand()) //vai dar erro